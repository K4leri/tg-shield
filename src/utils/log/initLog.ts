...
const generateTable = (whiteListChunks: string[]) => {
  const tableRows = [];
  tableRows.push(
    generateTableRow("Список супер админов без лимитов", whiteListChunks[0])
  );
  for (let i = 1; i < whiteListChunks.length; i++) {
    tableRows.push(
      `  | ${pad("", paramNameWidth)} | ${pad(whiteListChunks[i], paramValueWidth)} |`
    );
  }
...
export function logChatConfig(chatConfig: ChatConfigFromJson, index: number) {
  ...
  | ${pad("Отсылать капчу при вступлении", paramNameWidth)} | ${pad(chatConfig.sendCapctha ? "да" : "нет", paramValueWidth)} |
