...
class CaptchaManager {
  ...
  async sendCaptcha(
    chatId: number,
    user: User,
    captchaType: CaptchaType = this.standartType
  ) {
    ...
    switch (captchaType) {
      case CaptchaType.Math:
        ...
        timeoutDuration = 60000; // 20 seconds
        break;
      case CaptchaType.Audio:
        ...
        timeoutDuration = 60000; // 30 seconds
        break;
      default:
    ...
    const userNotif = html`${user.username
      ? `@${user.username},`
      : html`<a href="tg://user?id=${user.id}">${user.displayName}</a>,`}`;
    const caption = html`${userNotif} Посчитай правильное значение для
    вступления (${captchaType === CaptchaType.Math ? "60" : "60"} сек)`;

    const message = await bot.sendMedia(
      chatId,
      {
  ...
  clearTimers(userId: number) {
    ...
    if (captchaInfo) clearTimeout(captchaInfo.timeoutId);
  }
  ...
  changeCaptchaType(type: CaptchaType = this.standartType) {
    if (!isNaN(type)) {
      ...
      this.standartType =
        (this.standartType + 1) % (Object.keys(CaptchaType).length / 2);
    }
  ...
  async kickUser(chatId: number, userId: number) {
    ...
    if (!chatConfig)
      return logger.warn(`No chat config found for chat ${chatId}`);

    const message = await bot.kickChatMember({ chatId, userId });
    if (message && failedAttempts < chatConfig.maxFailedAttemps)
      messagesToDelete.push(message);

    this.userIds = this.userIds.filter((id) => id !== userId);
    await bot.deleteMessages(messagesToDelete);

    if (failedAttempts >= chatConfig.maxFailedAttemps)
      this.banUser(chatConfig, chatId, userId);
  }
