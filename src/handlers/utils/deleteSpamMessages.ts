...
export async function deleteSpamMessages(
  ...
  if (messageCount) {
    logger.info(
      `Приступил к удалению ${messageCount} сообщений о поступивших заявках`
    );
    const batchSize = 10;
    ...
    logger.info(
      `Удалил все сообщения и обновил действия в бд ${userId ? `для юзера ${userId}` : ""}`
    );
  }

  if (userId) {
    chatConfig.spamMessages = chatConfig.spamMessages.filter(
