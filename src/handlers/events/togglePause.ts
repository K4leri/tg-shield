...
export async function makeAdminAction(msg: MessageContext) {
  ...
  if (msg.text.includes("/type"))
    return captchaManager.changeCaptchaType(
      parseInt(msg.text.replace("/type", ""))
    );

  chatConfig.manualApproveMode = !chatConfig.manualApproveMode;
  if (chatConfig.manualApproveMode) {
    logger.warn(
      `Включил ручной режим работы в чате ${msg.chat.id}, автоматически данный режим отключится через ${chatConfig.hoursToOffManualMode / (60 * 60 * 1000)} часов`
    );
